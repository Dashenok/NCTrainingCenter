...
public class VectorMap<K extends  Object, V extends  Vector> implements Map<K, V> {
    ...
    private jl11.vector.Vector[] valueArr;
    ...
    public V put(K key, V value) {
        if ((containsKey(key))) {
            for (int i = 0; i < keyArr.length; i++)
                if (keyArr[i].equals(key)) {
                    jl11.vector.Vector prevVector = valueArr[i];
                    valueArr[i] = value;
                    return (V)prevVector;
                }
        } else {
            Object[] tempKeyArr = keyArr.clone();
            jl11.vector.Vector[] tempValueArr = valueArr.clone();
            keyArr = new Object[keyArr.length + 1];
            valueArr = new jl11.vector.Vector[valueArr.length + 1];
        ...
        if ((containsKey(key))) {
            for (int i = 0; i < keyArr.length; i++)
                if (keyArr[i].equals(key)) {
                    ...
                    keyArr = new jl11.vector.Vector[keyArr.length - 1];
    ...
    public void clear() {
        keyArr = new Object[0];
        valueArr = new jl11.vector.Vector[0];
    }
    ...
    public Collection values() {
        return new jl11.vector.container.VectorCollection<Vector>(valueArr);
    }
